function sample_data = EP_velocityMagDirPP( sample_data, qcLevel, auto )
...
for k = 1:length(sample_data)
  ...
  hasCSPD_CDIR = getVar(sam.variables, 'CSPD') || getVar(sam.variables, 'CDIR');
  if hasCSPD_CDIR, continue; end
  
  hasUCUR_VCUR = getVar(sam.variables, 'UCUR') || getVar(sam.variables, 'VCUR');
  hasUCUR_VCUR_MAG = getVar(sam.variables, 'UCUR_MAG') || getVar(sam.variables, 'VCUR_MAG');
  magExt = '';
  if hasUCUR_VCUR
      ucurIdx = getVar(sam.variables, 'UCUR');
      vcurIdx = getVar(sam.variables, 'VCUR');
  else
      ucurIdx = getVar(sam.variables, 'UCUR_MAG');
      vcurIdx = getVar(sam.variables, 'VCUR_MAG');
      magExt = '_MAG';
  end
  ...
  comment = 'EP_velocityMagDirPP.m: CSPD and CDIR were derived from UCUR and VCUR.';
