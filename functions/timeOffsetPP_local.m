function sample_data = timeOffsetPP_local(sample_data, qcLevel, auto)
%TIMEOFFSETPP Prompts the user for a time correction to the given data 
% sets (that is only used in plotting routine, data is not modified).
% Based on imos-toolbox timeOffsetPP.
%
  ...
  if ~auto
      ...
      for k = 1:nSample
          ...
          offsetFields(k) = uicontrol(...
              'Style',    'edit',...
              'UserData', k, ...
              'String',   num2str(offsets(k),'%13.9f'));
      end
