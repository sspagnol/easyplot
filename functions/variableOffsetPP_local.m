function sample_data = variableOffsetPP_local( sample_data, qcLevel, auto )
...
for k = 1:nSampleData
    nVars = length(sample_data{k}.variables);
    for m = 1:nVars
        defaultOffsets{k}{m} = num2str(sample_data{k}.variables{m}.EP_OFFSET);
        defaultScales{ k}{m} = num2str(sample_data{k}.variables{m}.EP_SCALE);
    end
end
...
diffHeaderLabelStr = 'Not Used';
        ...
        for m = 1:nVars
            notRelevantParams = {'TIMESERIES', 'PROFILE', 'TRAJECTORIES', 'TIME', 'LATITUDE', 'LONGITUDE', 'NOMINAL_DEPTH', 'BOT_DEPTH', 'DIRECTION', 'EP_TIMEDIFF', 'EP_DEPTH', 'EP_PSAL', 'LPF_PRES_REL', 'LPF_EP_DEPTH'};
