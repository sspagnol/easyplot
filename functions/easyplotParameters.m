function [value, tf] = easyplotParameters( short_name, field )
...
path = '';
if ~isdeployed, [path, ~, ~] = fileparts(which('easyplot.m')); end
if isempty(path), path = pwd; end
path = fullfile(path, 'functions');

fid = -1;
if isdeployed
    persistent ep_params; % we actually only need to read the parmeters file once and this will improve performances
end

if ~exist('ep_params', 'var') || isempty(ep_params)
    try
        fid = fopen(fullfile(path, 'easyplotParameters.txt'), 'rt');
        if fid == -1, return; end
        
        ep_params = textscan(fid, '%s%d%s%s%s%s%s%f%f%f%s', ...
...
names          = ep_params{1};
cfCompliance   = ep_params{2};
standard_names = ep_params{3};
uoms           = ep_params{4};
positives      = ep_params{5};
datums         = ep_params{6};
data_codes     = ep_params{7};
fillValues     = ep_params{8};
validMins      = ep_params{9};
validMaxs      = ep_params{10};
varType        = ep_params{11};
