...
instrument_model_shortname = {'RDI', ...
    'RDI NEMO', ...
    'SBE16PLUS V2', ...
    'TEMP', ...
    'SOLO T'};
...
for ii = 1:length(instrument_model)
    if ~isempty(regexp(sam.meta.instrument_model, instrument_model{ii}, 'once'))
        sam.meta.instrument_model_shortname  = regexprep(sam.meta.instrument_model, instrument_model{ii}, instrument_model_shortname{ii});
    end
end
